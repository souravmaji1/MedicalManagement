...
const [wellnessForm, setWellnessForm] = useState({
  ...
  oxygenSaturation: ''
     ...
     // Replace the existing permission checks section (around line 600-650) with:

// Get module access level using the same pattern as dashboard
...
// Get access level for medications module
const moduleAccess = getModuleAccess('medications');

// Define permissions based on module access level
const canViewMedications = moduleAccess !== 'none';
const canAddMedications = moduleAccess === 'edit' || moduleAccess === 'admin';
const canEditMedications = moduleAccess === 'edit' || moduleAccess === 'admin';
const canDeleteMedications = moduleAccess === 'admin';

// MAR permissions - use mar_full module permissions
const marAccess = getModuleAccess('medications'); // MAR is part of medications
...
// Plans/Documents permissions (for the PDF signing feature)
...
const handleWellnessEntry = async (e) => {
  ...
  try {
    ...
    const historyEntry = {
      ...
      updated_by_role: userProfile.role_name,
      user_id: user.id,
      update_type: 'wellness_entry_add',
      changes: newWellnessEntry,
      action: `Wellness entry added: ${wellnessForm.type} - ${wellnessForm.title}`,
      wellness_id: newWellnessEntry.id,
      wellness_type: wellnessForm.type,
      wellness_date: wellnessForm.date,
      provider: wellnessForm.provider,
      // Vital signs if applicable
      vital_signs: wellnessForm.type === 'vital_signs' ? {
        bloodPressure: wellnessForm.bloodPressure,
        heartRate: wellnessForm.heartRate,
        temperature: wellnessForm.temperature,
        weight: wellnessForm.weight,
        oxygenSaturation: wellnessForm.oxygenSaturation
      } : null,
    ...
    setIndividuals(prev => prev.map(ind => 
      ind.id === selectedIndividual.id 
        ? { ...ind, wellness_data: updatedWellness, update_history: newHistory } 
        : ind
    ));
...
const resetWellnessForm = () => {
 ...
 const handleAddMedication = async (e) => {
  ...
  try {
    ...
    const historyEntry = {
      ...
      updated_by_role: userProfile.role_name,
      user_id: user.id,
      update_type: 'medication_add',
      changes: newMedication,
      action: `Medication added: ${newMedication.medicationname} - ${newMedication.dosage}`,
      medication_id: newMedication.id,
      medication_name: newMedication.medicationname,
      medication_category: newMedication.category,
      is_prn: newMedication.prn,
    ...
    const updatedIndividuals = individuals.map(ind => 
      ind.id === selectedIndividual.id 
        ? { ...ind, medications: updatedMedications, update_history: newHistory } 
        : ind
    );
 ...
 const handleDeleteMedication = async (medicationId) => {
  ...
  try {
    ...
    // Get the medication being deleted
    const deletedMedication = medications.find(med => med.id === medicationId);
    ...
    const historyEntry = {
      ...
      updated_by_role: userProfile.role_name,
      user_id: user.id,
      update_type: 'medication_delete',
      changes: { deleted_medication: deletedMedication },
      action: `Medication deleted: ${deletedMedication?.medicationname} - ${deletedMedication?.dosage}`,
      deleted_medication_id: medicationId,
      medication_name: deletedMedication?.medicationname,
      medication_category: deletedMedication?.category,
      was_active: deletedMedication?.status === 'Active',
      original_prescriber: deletedMedication?.prescribedby,
      individual_name: `${selectedIndividual.firstname} ${selectedIndividual.lastname}`
    };
 ...
 const handleMARentry = async (e) => {
  ...
  try {
    ...
    const historyEntry = {
      ...
      updated_by_role: userProfile.role_name,
      user_id: user.id,
      update_type: 'mar_entry_create',
      changes: newMAREntry,
      action: `MAR entry recorded: ${newMAREntry.medication_name} - ${marEntry.status}`,
      mar_entry_id: newMAREntry.id,
      medication_id: marEntry.medicationid,
      medication_name: newMAREntry.medication_name,
      administration_status: marEntry.status,
      administered_time: marEntry.administered_time,
      route_used: marEntry.route_used,
      is_prn: medications.find(m => m.id === marEntry.medicationid)?.prn || false,
      prn_reason: marEntry.prn_reason_given,
      witnessed_by: marEntry.witnessed_by,
      refusal_reason: marEntry.refusedreason,
      hold_reason: marEntry.heldreason,
      mar_validated: marEntry.mar_validated,
      billing_validated: marEntry.billing_validated,
      individual_name: `${selectedIndividual.firstname} ${selectedIndividual.lastname}`
    };
