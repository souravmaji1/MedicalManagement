...
// Add these permission checks after the useUserProfile hook
...
// Get access level for billing_report module
const moduleAccess = getModuleAccess('billing_report');

// Define permissions based on module access level
const canViewBilling = moduleAccess !== 'none';
const canEditBilling = moduleAccess === 'edit' || moduleAccess === 'admin';
const canAdminBilling = moduleAccess === 'admin';

// Specific action permissions
const canCreateEncounters = canEditBilling;
const canEditEncounters = canEditBilling;
const canDeleteEncounters = canAdminBilling;
const canCreateBatches = canEditBilling;
const canSubmitClaims = canEditBilling;
const canManageServiceCodes = canAdminBilling;
const canManageAuthorizations = canEditBilling;
const canExportBilling = canViewBilling;
const canPrintReports = canViewBilling;
...
if (claimsError) {
  ...
  const handleSubmitClaim = async (claimId) => {
     ...
     if (!canSubmitClaims) {
    alert('You do not have permission to submit claims.');
    return;
  }
...
const handleEditEncounter = (encounter) => {
  ...
  if (!canEditEncounters) {
    alert('You do not have permission to edit encounters.');
    return;
  }
 ...
 const handleEncounterChange = (e) => {
  ...
  const handleSaveEncounter = async (e) => {
    ...
    e.preventDefault();
      ...
      if (!canCreateEncounters) {
    alert('You do not have permission to create encounters.');
    return;
  }
...
const handleCreateClaimFromBatch = async (batch) => {
  ...
  const handleSaveServiceCode = async (e) => {
    e.preventDefault();
      ...
      if (!canManageServiceCodes) {
    alert('You do not have permission to manage service codes.');
    return;
  }
...
if (!profileLoading && !canViewBilling) {
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-400 mb-6">
            You do not have permission to view the billing module. Please contact your administrator if you believe this is an error.
          </p>
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
            ...
            <p className="text-xs text-slate-600 mt-2">
              Module Access: <span className="text-red-400 font-bold">NONE</span>
            </p>
          </div>
        </div>
      </div>
  ...
  <button
    ...
    className="flex items-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-500 text-white px-5 py-3 rounded-xl font-bold hover:shadow-2xl hover:shadow-emerald-500/50 transition-all duration-300 hover:scale-105"
  >
    <Plus size={20} />
    Add Encounter
  </button>
) : moduleAccess === 'view' ? (
  <div className="flex items-center gap-2 px-4 py-2 bg-slate-700/50 text-slate-400 rounded-xl text-sm border border-slate-600">
    <Lock size={14} />
...
<td className="px-4 py-3">
  ...
  <button
    onClick={() => setShowBatchBuilder(true)}
    className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-xl font-semibold transition-all"
  >
    <FileCheck size={20} />
    Create Batch
  </button>
