...
import { 
  ...
  TrendingDown, ArrowUp, ArrowDown, Minus, Eye, Share2,User2Icon,
...
<div className="bg-slate-800/30 rounded-xl p-4">
  <h4 className="text-white font-bold mb-3 flex items-center gap-2">
    ...
    Recent Activity & Update History
  </h4>
  <div className="space-y-3">
    {/* Current Activity Status */}
    <div className="bg-slate-900/50 rounded-lg p-3 border-l-4 border-emerald-500">
      <p className="text-xs text-slate-500 font-semibold uppercase mb-2">Current Activity Status</p>
      <div className="space-y-2">
        ...
        {individual.updated_by && (
          <div className="flex items-center justify-between">
            <span className="text-slate-400 text-sm">Last Updated By</span>
            <span className="text-white font-semibold">{individual.updated_by}</span>
          </div>
        )}
        {individual.updated_by_role && (
          <div className="flex items-center justify-between">
            <span className="text-slate-400 text-sm">Updated By Role</span>
            <span className="text-emerald-400 text-sm font-semibold">{individual.updated_by_role}</span>
          </div>
        )}
      </div>
    </div>
    ...
    {/* Update History */}
    {fullData?.update_history && fullData.update_history.length > 0 && (
      <div className="bg-slate-900/50 rounded-lg p-3 border-l-4 border-purple-500">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs text-slate-500 font-semibold uppercase">Update History</p>
          <span className="text-xs bg-purple-900/30 text-purple-400 px-2 py-0.5 rounded-full font-bold">
            {fullData.update_history.length} updates
          </span>
        </div>
        
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {fullData.update_history.slice(0, 10).map((update, idx) => (
            <div 
              key={idx} 
              className="bg-slate-800/50 rounded-lg p-3 border border-slate-700/50 hover:border-purple-500/30 transition-all"
            >
              {/* Update Header */}
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                    update.update_type === 'create' ? 'bg-green-900/30' :
                    update.update_type === 'status_change' ? 'bg-orange-900/30' :
                    update.update_type === 'status_review' ? 'bg-blue-900/30' :
                    update.update_type === 'edit' ? 'bg-purple-900/30' :
                    'bg-gray-900/30'
                  }`}>
                    {update.update_type === 'create' ? <Plus size={14} className="text-green-400" /> :
                     update.update_type === 'status_change' ? <AlertCircle size={14} className="text-orange-400" /> :
                     update.update_type === 'status_review' ? <Eye size={14} className="text-blue-400" /> :
                     update.update_type === 'edit' ? <Edit2 size={14} className="text-purple-400" /> :
                ...
                <span className={`text-xs px-2 py-1 rounded-full font-bold ${
                  update.update_type === 'create' ? 'bg-green-900/30 text-green-400' :
                  update.update_type === 'status_change' ? 'bg-orange-900/30 text-orange-400' :
                  update.update_type === 'status_review' ? 'bg-blue-900/30 text-blue-400' :
              ...
              <div className="bg-slate-900/50 rounded-lg p-2 mb-2">
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    ...
                    <p className="text-white font-semibold">{update.updated_by || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">Role</p>
                    <p className="text-emerald-400 font-semibold">{update.updated_by_role || 'N/A'}</p>
                  </div>
                </div>
              </div>
              ...
              {update.action && (
                <div className="mb-2">
                  <p className="text-xs text-slate-400 italic">"{update.action}"</p>
                </div>
              )}
              ...
              {update.changes && Object.keys(update.changes).length > 0 && (
                <div className="bg-slate-900/50 rounded p-2">
                  <p className="text-xs text-slate-500 font-semibold mb-1">Changes Made:</p>
                  <div className="space-y-1">
                    {Object.entries(update.changes).map(([key, value], changeIdx) => (
              ...
              {/* Status Change Specific Info */}
              {update.update_type === 'status_change' && (
                <div className="mt-2 pt-2 border-t border-slate-700/50">
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-slate-400">Status:</span>
                    <span className={`px-2 py-0.5 rounded-full font-bold ${
                      update.previous_status === 'Active' ? 'bg-green-900/30 text-green-400' :
                      update.previous_status === 'Review' ? 'bg-yellow-900/30 text-yellow-400' :
                      'bg-red-900/30 text-red-400'
                    }`}>
                    ...
                    <span className={`px-2 py-0.5 rounded-full font-bold ${
                      update.changes?.status === 'Active' ? 'bg-green-900/30 text-green-400' :
                      update.changes?.status === 'Review' ? 'bg-yellow-900/30 text-yellow-400' :
                      'bg-red-900/30 text-red-400'
                    }`}>
                  ...
                  {update.effective_date && (
                    <div className="mt-1 text-xs text-slate-400">
                      Effective Date: {formatDate(update.effective_date)}
                    </div>
                  )}
                </div>
              )}
              ...
              {update.user_id && (
                <div className="mt-2 pt-2 border-t border-slate-700/50">
                  <p className="text-xs text-slate-500">
                    User ID: <span className="text-slate-400 font-mono">{update.user_id}</span>
                  </p>
                </div>
              )}
            </div>
        ...
        {/* Show More Link */}
        {fullData.update_history.length > 10 && (
          <div className="mt-3 text-center">
            <button 
              onClick={() => {
                // You can implement a modal or expanded view here
                alert(`Total ${fullData.update_history.length} updates in history`);
              }}
              className="text-xs text-purple-400 hover:text-purple-300 font-semibold flex items-center gap-1 mx-auto"
            >
              View All {fullData.update_history.length} Updates
              <ChevronDown size={12} />
            </button>
          </div>
        )}
    ...
    {(!fullData?.update_history || fullData.update_history.length === 0) && (
      <div className="bg-slate-900/50 rounded-lg p-4 text-center">
        ...
        <p className="text-slate-500 text-sm">No update history available</p>
      </div>
    )}
  </div>
</div>
