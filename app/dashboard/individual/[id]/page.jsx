...
const handleCommunityActivityEntry = async (e) => {
  ...
  try {
    ...
    const historyEntry = {
      ...
      updated_by_role: userProfile.role_name,
      user_id: user.id,
      update_type: 'community_activity_add',
      changes: newActivity,
      action: `Community activity added: ${newActivity.activity_type} at ${newActivity.community_location}`,
...
const handleChoiceAcknowledgmentEntry = async (e) => {
  ...
  try {
    ...
    const historyEntry = {
      ...
      updated_by_role: userProfile.role_name,
      user_id: user.id,
      update_type: 'choice_acknowledgment_add',
      changes: newAcknowledgment,
      action: 'Choice & Autonomy acknowledgment recorded',
      signature_type: newAcknowledgment.signature_type,
      signature_date: newAcknowledgment.signature_date,
      hcbs_compliant: true
    };
...
const deleteCommunityActivity = async (activityId) => {
  ...
  try {
    const deletedActivity = communityActivities.find(a => a.id === activityId);
    ...
    const historyEntry = {
      ...
      updated_by_role: userProfile.role_name,
      user_id: user.id,
      update_type: 'community_activity_delete',
      changes: { deleted_activity: deletedActivity },
      action: `Community activity deleted: ${deletedActivity?.activity_type} at ${deletedActivity?.community_location}`,
...
const deleteChoiceAcknowledgment = async (acknowledgmentId) => {
  ...
  try {
    const deletedAcknowledgment = choiceAcknowledgments.find(a => a.id === acknowledgmentId);
    ...
    const historyEntry = {
      ...
      updated_by_role: userProfile.role_name,
      user_id: user.id,
      update_type: 'choice_acknowledgment_delete',
      changes: { deleted_acknowledgment: deletedAcknowledgment },
      action: 'Choice acknowledgment deleted',
      deleted_acknowledgment_id: acknowledgmentId
    };
  ...
  try {
    ...
    // Get current data to track previous values
    ...
    Object.keys(formData).forEach(key => {
      if (JSON.stringify(formData[key]) !== JSON.stringify(currentData[key])) {
    ...
    const historyEntry = {
      ...
      changes: changes,
      action: 'Individual profile updated',
      previous_values: previousValues,
      fields_updated: Object.keys(changes).length
    };
    ...
    setIndividual({ ...formData, update_history: newHistory });
    setIsEditing(false);
  ...
  try {
    const updatedOutcomes = [...formData.outcomes, newOutcome];
    ...
    const historyEntry = {
      ...
      updated_by_role: userProfile.role_name,
      user_id: user.id,
      update_type: 'outcome_add',
      changes: newOutcome,
      action: 'New outcome added to Individual Support Plan'
    };
