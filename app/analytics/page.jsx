...
const AnalyticsPage = () => {
  ...
  const calculateAnalytics = () => {
    ...
    // Apply time range filter for time-based metrics
    const now = new Date();
    ...
    // HCBS Compliance (calculate based on community integration metrics)
    const hcbsCompliance = totalIndividuals > 0
      ? Math.round((communityOutings / (totalIndividuals * 4)) * 100) // Assuming 4 outings per month as target
      : 0;
                  ...
                  <div className="space-y-2">
                    {selectedIndividual.goals.slice(0, 3).map(goal => (
                      <div key={goal.id} className="flex items-center justify-between">
                        <p className="text-sm text-slate-300">{goal.description}</p>Continue18:11<span className="text-sm font-bold text-emerald-400">{goal.progress}%</span>
</div>
))}
</div>
) : (
