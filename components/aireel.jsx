...
export default function ImageToVideoReel() {
  ...
  try {
    ...
    const callsNeeded = Math.ceil(totalImages / maxImagesPerCall); // 3 calls (4 + 4 + 2)
    let allGeneratedImages = [];

    for (let i = 0; i < callsNeeded; i++) {
      // Calculate how many images to request in this call
      const remainingImages = totalImages - allGeneratedImages.length;
      const numOutputs = Math.min(maxImagesPerCall, remainingImages);
      ...
      // Add the generated images from this call to the collection
      allGeneratedImages = [...allGeneratedImages, ...(data.output || [])];

      // If we've collected enough images, break early
      if (allGeneratedImages.length >= totalImages) {
        break;
      }
    }

    // Update state with the collected images (limit to 10 in case of overfetch)
    setGeneratedImages(allGeneratedImages.slice(0, totalImages));
  ...
  >
    ...
    {/*  <div className="md:w-1/2 mb-4">md:w-1/2
        <label className="block text-sm font-medium text-gray-300 mb-2">Background Music</label>
        <div className="bg-gray-900/70 border border-gray-700 rounded-xl p-6 h-full flex flex-col items-center justify-center relative overflow-hidden">
         
          <div className="absolute inset-0 bg-gray-900/80 backdrop-blur-sm flex flex-col items-center justify-center">
            <div className="relative">
              <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-600 to-blue-600 rounded-lg blur opacity-70 animate-pulse"></div>
              <div className="relative bg-gray-900 rounded-lg px-6 py-4 flex items-center justify-center">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400 font-bold text-xl">
                  Coming Soon
                </span>
              </div>
            </div>
          ...
          <div className="flex-1 space-y-3 opacity-20 pointer-events-none w-full">
            {['Energetic', 'Cinematic', 'Lofi', 'Ambient'].map((genre) => (
              <motion.div
                key={genre}
                className="flex items-center gap-3 p-3 bg-gray-800/60 rounded-lg cursor-not-allowed group border border-gray-700/50"
              >
            ...
            <motion.button
              className="mt-3 border border-dashed border-gray-600 text-gray-400 rounded-lg py-3 w-full flex items-center justify-center gap-2"
            >
              <Upload size={16} />
              Upload Music
            </motion.button>
